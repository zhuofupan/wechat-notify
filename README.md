...
![三种通知的实际样式](docs/message-styles.png)

**💻 设备名**：取 OS 主机名（`platform.node()`，与 WorkBuddy 心跳记录的 `hostname` 一致）。同一账号多台设备登录时，一眼分清消息来自哪台机器。

**📝 完成概要**：取助手最后一条消息，**自动去掉无用字符**——Markdown 装饰（`**`、`#`、表格线、链接只留文字）、客套话开场白（"完成。/搞定了。/好的。/明白了。/让我…"等动词+了形态整体剥离）、空行与纯符号行——再挑信息量最高的一句裁到 40 字（`--stop-summary-chars` 可调）。若剥离后句子残缺（如只剩"了。…"），整句弃用换下一句。
